...
invalid_a_p db 13,10,"You have entered an invalid username or password...Try again!!! $"
...
dcode_err db 13,10,"Invalid Dish Code. Enter Single Digit Dish Code $"
dname_err db 13,10,"Invalid Dish Name. Enter Single Digit Dish Code $"
dprice_err db 13,10,"Invalid Dish Price. Enter Single Digit Dish Code $"

;Display Dish Data
d_infostr1 db "Dish Information $"
d_infostr2 db "Dish Code: $"
d_infostr3 db "Dish Name: $"
d_infostr4 db "Dish Price: $"
...
int 21h
cmp al,0dh    
je end_unwhile  ;if al=enter key ascii then goto label end_while
...
compareu:          ;comparing two strings character by character 
mov al,[bx]
cmp al,[si]
jne not_uequal  
inc bx
...
valid2:
     cmp bl,31h
     je add_d   ;if bl==1 then goto create_acc label
          
     cmp bl,32h
     je search_d   ;if bl==2 then goto search_acc label
         
     cmp bl,33h
     je delete_d   ;if bl==3 then goto delete_acc label
     
     cmp bl,34h
     je display_d  ;if bl==4 then goto show_userss label
...
cmp al,31h
je sd            ;if al==1 then goto admin label
cmp al,32h
je mm            ;if al==2 then goto customer label
...
err:
lea dx,ds_menustr5 ;Dish Menu Error
mov ah,09h       
int 21h
...
cmp bl,32h
je admin_main_menu   ;if bl==2 then goto admin main menu label
